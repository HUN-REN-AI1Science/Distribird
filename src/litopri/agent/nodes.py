...
async def search_node(state: PipelineState) -> dict:
    ...
    if settings.enable_deliberation:
        ...
        agent_findings = await run_source_agents(
            parameter, queries, settings, enrichment,
        )
        deliberation = await deliberate(
            agent_findings, parameter, settings, enrichment,
        )
...
async def extract_node(state: PipelineState) -> dict:
    ...
    papers_with_values = extract_all_values(
        papers, parameter, settings, enrichment=enrichment,
    )
    ...
    if total_values == 0 and papers and settings.llm_web_search:
        ...
        if web_papers:
            papers_with_values.extend(
                p for p in web_papers if p not in papers_with_values
            )
            total_values = sum(
                len(p.extracted_values) for p in papers_with_values
            )
            warnings.append(
                "Used web-assisted extraction to look up paper content online."
            )

    traces.append(_trace("extract", t0, {
        "n_papers": len(papers),
...
async def quality_gate_node(state: PipelineState) -> dict:
    ...
    traces.append(_trace("quality_gate", t0, {
...
async def synthesize_node(state: PipelineState) -> dict:
    ...
    traces.append(_trace("synthesize", t0, {
...
async def refine_search_node(state: PipelineState) -> dict:
    ...
    paper_summaries = "\n".join(
        f"- {p.title} ({p.year}): {(p.abstract or '')[:150]}"
        for p in papers[:10]
    )
    bb_text = "\n".join(
        f"- [{m.kind.value}] {m.sender}: {m.content}"
    ...
    if isinstance(raw, dict):
        ...
        new_queries = [
            q for q in raw.get("new_queries", [])
            if isinstance(q, str) and q not in all_tried
        ]
        term_updates = raw.get("terminology_updates", [])

        if diagnosis:
            post_message(
                state, "refine_search", MessageKind.WARNING,
                f"Search refinement diagnosis: {diagnosis}",
            )
        for term in term_updates:
            post_message(
                state, "refine_search", MessageKind.TERMINOLOGY,
                term,
            )
    ...
    traces.append(_trace("refine_search", t0, {
...
async def cross_enrich_node(state: PipelineState) -> dict:
    ...
    key_papers_text = "\n".join(
        f"- {p.title} ({p.year}, DOI: {p.doi}): {(p.abstract or '')[:200]}"
        for p in key_papers
    )
    ...
    for p in key_papers:
        if p.doi:
            post_message(
                state, "cross_enrich", MessageKind.CROSS_REF,
    ...
    traces.append(_trace("cross_enrich", t0, {
...
async def refine_extraction_node(state: PipelineState) -> dict:
    ...
    if candidates:
        web_papers = extract_values_web_assisted(
            candidates[:10], parameter, settings, enrichment=enrichment,
        )
        if web_papers:
            papers_with_values.extend(
                p for p in web_papers if p not in papers_with_values
            ...
            warnings.append(
                "Used refined extraction on high-relevance papers."
            )

    budget.extraction_refinement_used += 1
    budget.total_llm_calls_used += 1

    traces.append(_trace("refine_extraction", t0, {
